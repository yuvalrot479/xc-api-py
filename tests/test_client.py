from xeno_canto import Client, SearchQuerySchema, tags
from dotenv import dotenv_values
from rich import print
import pytest
import requests_mock
from xeno_canto.clients import Client, ClientError, ServerError
from xeno_canto.schemas.search_query import SearchQuerySchema
...
def mock_response():
  ...
  return {
    ...
    'page': 1,
    'numPages': 1,
    'recordings': [{'id': '12345', 'gen': 'Anas', 'sp': 'platyrhynchos', 'en': 'Mallard', 'type': 'song'}],
  }
...
def test_find_one_success(client, mock_response):
  with requests_mock.Mocker() as m:
    m.get('https://xeno-canto.org/api/3/recordings', json=mock_response)
    query = SearchQuerySchema(animal_genus='Anas')
...
def test_lean_validation(client, mock_response):
  ...
  with requests_mock.Mocker() as m:
    ...
    from xeno_canto.schemas.recording import XenoCantoRecordingBaseSchema

    assert isinstance(res, XenoCantoRecordingBaseSchema)
