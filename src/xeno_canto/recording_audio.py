from .schemas.recording import XenoCantoRecordingSchema
...
class RecordingAudio:
      ...
      if not self._data:
        if self._fetch_promise:
          ...
          if not self._target_sr:
            raise RuntimeError()
          self._decoded = miniaudio.decode(data=self._fetch_promise(), sample_rate=self._target_sr)

        elif self._original_file:
          # .from_file(...)
          self._decoded = miniaudio.decode_file(filename=str(self._original_file))
  ...
  @classmethod
  def from_recording(cls, recording: XenoCantoRecordingSchema):
    obj = cls()
    obj._fetch_promise = partial(_fetch, cast(yarl.URL, recording.recording_file_url))  # HACK
